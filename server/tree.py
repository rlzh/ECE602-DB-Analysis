...
def build_tree(df, cols, predict_attr, pp = 0, pn = 0):
    ...
    if p == 0 or n == 0:
        ...
        tree.left = build_tree(sub_1, cols, predict_attr, p, n)
        tree.right = build_tree(sub_2, cols, predict_attr, p, n)
        return tree
...
def test_predictions(root, df, y):
    num_data = df.shape[0]
    num_correct = 0
...
def test_f1score(root, df, y):
    ...
    for index,row in df.iterrows():
        ...
        if prediction == 1 and row[y] == 1:
            tp += 1
        elif prediction == 1 and row[y] == 0:
            fp += 1
        elif prediction == 0 and row[y] == 1:
            fn += 1
    print("tp: %f, fp: %f, fn: %f" % (tp,fp,fn) )
    f1score = 2*tp/(2*tp+fp+fn)
    return round(f1score, 2)
...
def train_test_split(dataset, split=0.90):
    ...
    #print(dataset)
    train, test = np.split(dataset, [int(split*len(dataset))])
    return train, test
...
#     print(str(test_predictions(root, df_test, y)*100.0) + '%')
